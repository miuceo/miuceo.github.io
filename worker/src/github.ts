...
export function isAllowedPath(path: string): boolean {
  // The editor's own store: the block source of every post, and its index.
  if (path === 'posts.json') return true;
  if (/^posts-data\/[a-z0-9-]+\.json$/.test(path)) return true;

  // The published content, one file per language. Written only by an
  // authenticated author action — publishing, or approving a translation.
  // The agent cannot reach this function at all.
  if (/^src\/content\/posts\/[a-z0-9-]+\/(uz|en|ru)\.md$/.test(path)) return true;

  // v1's rendered pages are no longer generated, but the already-published
  // ones stay reachable and admin.html still has to be able to delete them.
  // Deliberately no longer writable — nothing writes here any more, and a
  // path that only needs deleting does not need to stay writable to be
  // deletable through ghDeleteFile.
  if (/^posts\/[a-z0-9-]+\.html$/.test(path)) return true;

  // An artifact's metadata (title, model, tags, ...) — low-risk structured
  // JSON, same posture as posts-data above.
  if (/^src\/content\/artifacts\/[a-z0-9-]+\.json$/.test(path)) return true;

  // The artifact's own HTML is deliberately absent from this generic
  // allowlist. It is arbitrary third-party markup that ends up in a
  // sandboxed iframe on the public site, so it only goes through the
  // dedicated /api/artifacts/save handler, which size-caps it and scans for
  // leaked secrets before calling ghPutFileSafe directly — a path this
  // generic put/get/delete trio can't enforce.

  // rss.xml is generated by the Astro build now (src/pages/rss.xml.ts), not
  // written by a client, so it is deliberately absent from this list.
  return false;
}

/** Paths writable only through the dedicated, validated /api/artifacts/save
 * handler — never through the generic /api/github/put (see isAllowedPath). */
export function isArtifactFilePath(path: string): boolean {
  return /^public\/artifacts\/files\/[a-z0-9-]+\.html$/.test(path);
}
