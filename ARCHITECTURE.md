...
- **Not done / explicit follow-ups, not silently rolled into this pass:**
  1. GitHub Pages' "Build and deployment" source has **not** been switched from branch/root to "GitHub Actions" — the live site is still v1, unchanged. This is the actual cutover moment and needs a deliberate decision, not an automatic one.
  2. The Cloudflare Web Analytics beacon token in `BaseLayout` is a placeholder (`REPLACE_WITH_CF_BEACON_TOKEN`) — needs the real token from the author's Cloudflare dashboard.
